...
transmit_packet(void *buf, size_t size)
{
	...
	tx_ring[tail].addr = (uint64_t) PADDR(tx_buffers[tail]); //TODO: Redundant! Put this in the initialization
...
init_receive(void)
{
	...
	for (i = 0; i < NUM_RX_DESC; i++) {
		...
		// Buffer address
		rx_ring[i].addr = (uint64_t) PADDR(rx_buffers[i]);
	}
...
attach_e1000(struct pci_func *pcif)
{
	...
	uintptr_t pa = pcif->reg_base[0];
	size_t size = pcif->reg_size[0];
	e1000 = ioremap(pa, size);
